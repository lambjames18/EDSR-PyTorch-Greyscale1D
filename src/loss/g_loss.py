...
class GLoss(nn.Module):
    ...
    def forward(self, output, target):
        ...
        g_loss = F.mse_loss(g_target, g_output)

        # calculate the pixel loss
        p_loss = F.mse_loss(output, target)

        # downsample the images
        target_down = self.max_pool(target)
        output_down = self.max_pool(output)
        ...
        g_d_loss = F.mse_loss(g_target_down, g_output_down)
